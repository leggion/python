...
# TODO: venv\Scripts\activate.bat
# TODO: python -m streamlit run your_script.py
...
with col1:
    st.title("Определение сонливости водителя!!!🥱😪😴")
    with st.container():
        c1, c2 = st.columns(spec=[1, 1])
        with c1:
            # The amount of time (in seconds) to wait before sounding the alarm.
            WAIT_TIME = st.slider("Секунды ожидания, прежде чем подать сигнал тревоги: 1.00:", 0.0, 5.0, 1.0, 0.25)

        with c2:
            # Lowest valid value of Eye Aspect Ratio. Ideal values [0.15, 0.2].
            EAR_THRESH = st.slider("Порог соотношения сторон глаз:", 0.0, 0.4, 0.18, 0.01)
